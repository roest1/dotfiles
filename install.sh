...
if [[ ${#SECTIONS[@]} -eq 0 ]] || printf '%s\n' "${SECTIONS[@]}" | grep -qx bash; then
  ...
  if [[ ! -f "$local_bash_profile" ]]; then
    # Kept byte-identical to the tracked bash/bash_profile, including the
    # shebang and shellcheck directive — the configs are linted now, so a
    # generated file that differs would pass CI in the repo and fail it on
    # the machine that regenerated it.
